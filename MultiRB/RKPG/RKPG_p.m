function [X1, X2, final_err, vec_res, it, inner_it, avg_inner, V] = RKPG_p(A, rhs1, rhs2, Q, d, poles, tol, maxit)
...
%         V = get_ek_basis(A, V);
        V = get_rk_basis_p(A, Q, d, poles(i), V);
...
%         X_hat = V*Y*V';
        ...
        vec_res(it) = res;
        if it > 1
        if (vec_res(it) > vec_res(it-1) && vec_res(it) < 1e-6)
            break
        end
        end
    end
