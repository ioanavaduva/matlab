...
%SUBSREF   Evaluate an RKFUNM (calls feval).
...
if length(varargin{1}.subs) == 1
  w = feval(obj, varargin{1}.subs{1}); % evaluate at z
  return
end
if length(varargin{1}.subs) == 2
  w = feval(obj, varargin{1}.subs{1}, varargin{1}.subs{2}); % eval at z, limit degree
  return
end
