...
while it <= param.maxit+1
    ...
    if param.k >= 0
        Qnum = eye(m+1+param.k);
    else
        Qnum = Qnum(1:m+1+param.k, :)';
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if it == param.maxit+1% || m == 0
        break;
    end
    if local_misfit < param.tol && reduction_done
        break;
    end
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Extend space if Greedy %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    if param.greedy && m < N-1 && local_misfit >= param.tol
        m = m + 1;
        xi = [xi, inf];
        [V, K, H] = rat_krylov(A, V, K, H, inf);
        if param.k < 0
            [~, ~, Qnum] = util_hh2th(K, H);
        end
        if param.k >= 0
    ...
    if local_misfit < param.tol
        [~, s, W] = svd(S, 0);
        s = diag(s);
        reduce_denominator; % break on next iteration
    else
        [~, ~, W] = svd(S, 0);
        W = W(:, end:-1:1);
...
if return_best && out.return_it ~= it
    coeffs = best_coeffs;
    ratfun = best_ratfun;
    V = best_V; K = best_K; H = best_H; xi = best_xi; m = best_m;
...
if ~isfield(param, 'tol'),       param.tol = 1e-14;      end
if ~isfield(param, 'k'),         param.k = 0;            end
...
if ~isfield(param, 'greedy'),    param.greedy = false;   end
