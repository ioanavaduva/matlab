function R = util_nleigs(A, Sigma, Xi, tol, Nmax, cyclic)
% UTIL_NLEIGS    Runs the NLEIGS sampling procedure for a 
%                nonlinear eigenvalue problem, returning 
%                an RKFUNM object.
%
% Input arguments:
...
% tol       - tolerance for the construction of the interpolant
% Nmax      - maximal degree of the interpolant
% 'cyclic'  - whether or not to repeat the poles in Xi cyclically (0 or 1)
% 
% Output arguments:
% R         - an RKFUNM object representing a rational interpolant.
%
% The NLEIGS sampling procedure is based on rational Newton interpolation
% and described in 
%
...
% the poles to Xi-s.
  ...
  AB.eval = @(z,maxN) lin_nlep_eval(AB,z,maxN);
...
function QN = lin_nlep_eval(AB,z,maxN)
