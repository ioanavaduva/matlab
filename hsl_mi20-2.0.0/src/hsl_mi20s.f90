...
module hsl_mi20_single ! 11 sept 2007
 ...
 implicit none
  ...
  integer, parameter, private :: myreal = kind(1.0)
   ...
   if (size_clevel .gt. 1) then
      ...
      if (control%coarse_solver .eq. 4) then
         ...
         if (.not. keep%lapack_data) then
            ...
            call sgetrf(size_clevel, size_clevel, keep%lapack_factors, &
 ...
 subroutine amg_v_cycle(matrix, coarse_data, max_level, vec_storage, &
   ...
   if (vec_length == 1) then
      ...
      if (control%coarse_solver == 2) then
         ...
         call sgetrs('n', vec_length, 1, keep%lapack_factors, vec_length,  &
...
! todo :: print out krylov solver being used
 ...
 subroutine simple_iteration(matrix,coarse_data,rhs,sol, & 
    ...
    real(myreal) :: snrm2     ! BLAS function
    ...
    norm_res_0 = snrm2(matrix%m,res,1)
...
!       call daxpy(matrix%m,-1.0,temp,1,res,1)
  ...
  subroutine solve_cg(matrix,coarse_data,rhs,sol, & 
    ...
    external mi21i, mi21a
    
    call mi21i(icntl, cntl, isave, rsave)
    ...
    iact = 0
    do
       call mi21a(iact,matrix%m,w,matrix%m,locy,locz,resid,icntl,cntl,& 
            info21,isave, rsave)
       if (iact == -1) then
          if (error_out) then 
...
!    integer :: restart
    ...
    external mi24i, mi24a
    
    call mi24i(icntl, cntl, isave, rsave, lsave)
    ...
    iact = 0
    do
       call mi24a(iact,matrix%m,solve_control%gmres_restart, & 
...
!    integer :: restart
    ...
    external mi26i, mi26a
    
    call mi26i(icntl, cntl, isave, rsave)
    ...
    iact = 0
    do
       call mi26a(iact,matrix%m,w,matrix%m,locy,locz,resid,&
