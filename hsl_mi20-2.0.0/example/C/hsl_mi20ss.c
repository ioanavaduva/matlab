...
void mi21i_(int *icntl, float *cntl, int *isave, float *rsave);
void mi21a_(int *iact, const int *n, float *w, const int *ldw, int *locy,
      int *locz, float *resid, int *icntl, float *cntl, int *info, int *isave,
      float *rsave);

/* Generate example matrix */
void matrix_gen(const int n, int **ptr, int **col, float **val) {
   int i,nnz,p;
...
/* Calculate b = Ax */
void spmv(int n, const int ptr[], const int col[], const float val[],
      float b[], const float x[]) {
   int i, j;
...
int main(void) {
   ...
   iact = 0;
   while(1) {
      mi21a_(&iact, &n, w, &n, &locy, &locz, &resid, icntl, cntl, info21,
         isave, rsave);
      if (iact == -1) {
